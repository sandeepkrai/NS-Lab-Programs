...
main (int argc, char *argv[])
{
  ...
  Ipv4Address multicastGroup ("225.1.3.5");
  ...
  multicastRouter = c.Get (5);
  inputIf = nd3.Get (0);
  NetDeviceContainer outputDevices2;
  outputDevices2.Add (nd2.Get (1));

  multicast.AddMulticastRoute (multicastRouter, multicastSource, multicastGroup, inputIf, outputDevices2);

  

  // Ptr<Node> multicastRouter2 = c.Get (5);  // The node in question
  // Ptr<NetDevice> inputIf2 = nd3.Get (0);  // The input NetDevice
  // NetDeviceContainer outputDevices1;  // A container of output NetDevices
  // outputDevices.Add (nd3.Get (0));
  // outputDevices.Add (nd3.Get (1));
  ...
  ApplicationContainer sinkC = sink.Install (c.Get (5)); // Node n5 
  // Start the sink
  ...
  csma.EnableAsciiAll (ascii.CreateFileStream ("LAB4/l4_q3.tr"));
  p2p.EnableAsciiAll (ascii.CreateFileStream ("LAB4/l4_q3.tr"));
  AnimationInterface anim ("LAB4/l4_q3.xml");
