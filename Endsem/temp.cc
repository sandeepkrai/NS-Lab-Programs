...
uint32_t vedika = 0;
CommandLine cmd;
cmd.AddValue("vedika", "MidSem Marks", vedika);
cmd.Parse (argc, argv);
std::cout<<vedika;
...
AsciiTraceHelper ascii;
    pointToPoint1.EnableAsciiAll(ascii.CreateFileStream("temp.tr"));
    pointToPoint2.EnableAsciiAll(ascii.CreateFileStream("temp.tr"));
AnimationInterface anim("temp.xml");
Simulator::Run ();
Simulator::Destroy ();
return 0;
}